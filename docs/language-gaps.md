...
The registry's `FULL_CURRENT_SUPPORT` currently names all builtin languages: Python, C, C++, JavaScript/TypeScript/TSX, Rust, Go, Java, Kotlin, and C#. However, a green capability flag is not the same as deep behavioral coverage; many languages implement a conservative slice for a flag. This audit records the behavioral gaps that remain after the flag is set.
