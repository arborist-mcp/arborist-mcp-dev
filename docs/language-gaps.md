...
| Shell | No sourced-file/command graph; only `function_definition` declarations produce semantic skeletons and declaration indexing; no variable/alias/source resolution or file dependencies; structural patch targeting and conservative same-file direct command tracing resolve only same-file function calls; patch reference validation rejects unknown and ambiguous same-file targets and ignores string/expansion/path command names | Keep conservative slice; add fixtures for sourced-file and variable-command resolution when real |
| Bash | Same conservative slice as Shell, sharing the `tree-sitter-bash` adapter; no bash-specific builtin, alias, or exported-function graph | Keep parity with Shell |
