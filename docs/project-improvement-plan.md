...
- [x] Reuse one exact `TraceQueryDeadline` across nested in-memory trace,
  list, search, and read-context execution; source-overlay trace dispatch; and
  outer live, persisted-index, and override list, search, direct or position
  read, or trace expansion, instead of recreating rounded child budgets.
