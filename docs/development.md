...
The target splits generated bytes into Python source and a Tree-sitter query,
then invokes the public query API with a fixed capture limit. The standalone
fuzz manifest can be type-checked on stable Rust with:
