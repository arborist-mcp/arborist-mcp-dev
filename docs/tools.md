...
- Go: `.go` — Tree-sitter parsing, raw queries, semantic skeletons, and conservative declaration
  ...
  `Counter{}.Value()` to one matching production method in the same local package, or through the caller method's own unshadowed named receiver. Module-root imports, external
