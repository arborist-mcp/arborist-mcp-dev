...
- Kotlin: `.kt`, `.kts` — Tree-sitter parsing, raw query execution, package-qualified semantic skeletons, and declaration indexing for top-level and nested classes, interfaces, enums, named objects, functions, simple properties, and type aliases. Local declarations inside function bodies are intentionally omitted because they have no stable file-level semantic path. Kotlin now refreshes unique explicit import dependents against `.kt` files that declare the imported package and traces unqualified direct calls to enclosing-type functions, same-package top-level functions, or unique explicitly imported top-level functions from other packages, and traces qualified receiver calls whose receiver type is pinned to a local class, interface, or type alias by a constructor initializer, explicit type annotation, parameter type, or enclosing-class property; named-object receivers such as `Config.helper(...)` dispatch to the object's members when the name resolves to a uniquely declared same-package or explicitly imported object, class-name receivers such as `Config.helper(...)` dispatch to companion-object members when the name resolves to a uniquely declared class or interface and the member is declared in its companion object while instance members and unknown companion members fail closed, explicit companion chains such as `Config.Companion.helper(...)` dispatch directly to companion members when the class name resolves to a uniquely declared type and no local binding shadows it while instance members, unknown companion members, and extension fallbacks fail closed, and explicit companion chains on classes declared in an explicitly imported package such as `import org.util.Config` with `Config.Companion.helper(...)` dispatch directly to the imported class's canonical companion member across packages while instance members and unknown companion members fail closed, named companion objects such as `companion object Factory` are indexed under their declared name so calls such as `Config.Factory.helper(...)` or `Config.Factory.holder.run(...)` dispatch through the same canonical companion scope as the `Companion` spelling while unknown companion names and companion chains rooted at object declarations fail closed, and enum classes declared in an explicitly imported package such as `enum class Color` with `companion object Factory` dispatch both the declared-name and canonical `Companion` spellings to the imported enum's companion scope across packages while unknown companion names and instance members reached through the class name fail closed, and named companion objects on regular classes declared in an explicitly imported package such as `import org.util.Config` with `companion object Factory` dispatch both the declared-name and canonical `Companion` spellings to the imported class's canonical companion scope across packages while unknown companion names and instance members reached through the named companion fail closed, companion property chains such as `Config.Companion.holder.run(...)` resolve each intermediate property's declared type within the companion scope before dispatching the final member or extension while chains through instance properties fail closed, nested companion receivers such as `Outer.Inner.helper(...)`, `Outer.Inner.Companion.helper(...)`, or `Outer.Inner.Factory.holder.run(...)` dispatch through the canonical companion scope of a nested class or interface when the first hop resolves to a uniquely declared type and the second hop names exactly one nested class or interface that hosts a companion while nested types without companions, unknown or ambiguous nested types, and locally shadowed outer names fail closed, and chained receiver calls such as `group.member.helper(...)` or `Config.holder.run(...)` additionally resolve each intermediate property's declared type, falling back to a bare-identifier constructor initializer such as `val member = Other()` when the property has no explicit type, or to the declared return type of a uniquely resolved same-file, same-package, or explicitly imported initializer function such as `val member = makeOther()`, a dotted factory return type such as `fun makeInner(): Outer.Inner` pins a nested receiver through the same dotted type-path rules while missing nested targets and factories without a declared return type fail closed, and a local receiver binding such as `val other = makeOther()` resolves the same way through the factory's declared return type, with the first hop either locally bound or a named object, before dispatching the final member or extension, and generic declared types such as `Box<String>`, generic property hops such as `val member: Box<Entry>`, generic method-return hops such as `fun inner(): Box<Entry>`, generic factory returns such as `fun makeBox(): Box<Entry>`, and generic superclass specifiers such as `class Derived : Base<Entry>()` normalize to their raw dotted base types before dispatch while generic arrays such as `Array<Helper>` stay capability-gated for the array slices, and nullable declared spellings such as `Box?`, `Outer.Inner?`, or `Box<String>?` normalize to the same raw base types for bound parameters, property hops, method-return hops, factory returns, and factory-inferred local bindings while nullable generic arrays such as `Array<Helper>?` and nullable value types such as `Int?` fail closed, and when the pinned type has no matching member, an unambiguous top-level extension function for that receiver type declared in the same file, the same package, or explicitly imported resolves the call, while member functions shadow extensions and qualified non-constructor initializers, function-call initializers whose function has no declared return type or is unknown or ambiguous, unknown chain hops, and ambiguous targets fail closed, and bare calls to constructible class names such as `Other(...)` resolve to the class declaration through the same scope/import rules, qualified nested constructors such as `Outer.Inner(...)` and nested receiver paths such as `Outer.Inner` in local bindings, property initializers, declared property types, or parameter types resolve through the same dotted type-path rules, dotted type-alias targets such as `typealias Helper = Outer.Inner` expand through the same rules while missing nested targets and cyclic alias chains fail closed, and type aliases declared in or imported from another package such as `import org.util.Helper` with `typealias Helper = Entry` expand their target in the alias's own file and package scope so the trailing member dispatches across packages while unresolvable or generic alias targets fail closed, and a property hop whose declared type is a type alias declared in an explicitly imported package such as `val member: Helper` with `typealias Helper = Other` resolves the alias target in the alias's own file and package scope before dispatching the trailing member across packages while unresolvable alias targets fail closed, nested object receivers such as `Outer.Inner.helper(...)` dispatch to the nested object's members when the first hop resolves to a uniquely declared class or object and the second hop names exactly one nested object declaration while unknown or ambiguous nested objects, nested classes or interfaces that share a nested object's name, and locally shadowed outer names fail closed, and constructor-call receivers such as `Outer.Inner().helper(...)` or `Group().member.helper(...)` resolve the constructed type path through the same constructible-class rules and then dispatch the member chain like any other instance receiver while a bare factory-call root such as `makeGroup().entry.helper(...)` or `makeInner().helper(...)` resolves the leading call through the same factory rules as a `var` initializer (a unique same-file, same-package, or explicitly imported top-level function with a declared return type, including dotted nested return types such as `Outer.Inner`) and dispatches the trailing member chain on the factory's declared return type, while unknown or missing types, unknown or ambiguous factories, factories without a declared return type, dotted factory roots, and non-constructible bases fail closed, and interfaces, enums, and sealed/abstract/annotation/inner classes, unknown or missing nested types, and nested type aliases fail closed. An element-access receiver such as `items[0].helper(...)` on a single-level generic array-typed parameter, local property, or enclosing-class property such as `items: Array<Helper>` dispatches the final member on the array's element component type when the base binds uniquely with a usable single-level component, including a simple identifier subscript such as `items[index].helper(...)`, while a direct member call on the array itself such as `items.helper(...)`, primitive-component arrays such as `counts[0].helper(...)`, multi-dimensional arrays such as `matrix[0][0].helper(...)`, member chains after an element hop, and unbound element-access bases fail closed. A bare factory-call element-access receiver such as `makeItems()[0].helper(...)` resolves the leading call through the same factory rules as a property initializer (a unique same-file, same-package, or explicitly imported top-level function with a declared return type) and dispatches the final member on the factory return array's element component type, and a qualified factory-call element-access receiver such as `Util.makeItems()[0].helper(...)` or `group.makeGroups()[0].helper(...)` resolves the leading call through the same qualified-initializer rules (companion, explicit-companion, object, or bound-receiver member), while unknown factories, overloaded factories, unresolvable qualified callees, `this`- or `super`-rooted factory callees, primitive- or multi-dimensional-returning factories, non-array-returning factories, and multi-dimensional element access on a factory-returned array fail closed. A `val` local bound from a single-level element access such as `val first = items[0]` inherits the base array's element component type when the base is an array-typed parameter, local property, or enclosing-class property such as `items: Array<Helper>`, and dispatches the final member on that element type, while primitive-component bases such as `val fromCounts = counts[0]`, multi-dimensional element access such as `val fromMatrix = matrix[0][0]`, unknown bases, and plain identifier initializers fail closed. The same `val` element-access binding applies when the element component type is declared in an explicitly imported package, so the final member dispatches across packages, while primitive-component bases, multi-dimensional element access, unknown bases, and unresolvable imported element types still fail closed. A `val` bound from an element access with a qualified base such as `val first = group.fieldItems[0]` or `val second = group.holder.fieldItems[0]` walks each intermediate property's declared type and dispatches the final member on the terminal array field's element component type, while `this`-rooted bases, method-call bases, unknown receivers, and primitive- or multi-dimensional-component bases fail closed. The same qualified `val` element-access binding applies when the bound and hop types are declared in an explicitly imported package, so each intermediate property's declared type resolves in the declaring type's own package and the final member dispatches on the terminal array field's imported element component type, while `this`-rooted bases, method-call bases, unknown receivers, unknown hops, and primitive- or multi-dimensional-component bases still fail closed. A `val` bound from a factory-call element access such as `val first = makeItems()[0]` resolves the leading call through the same factory rules as a property initializer (a unique same-file, same-package, or explicitly imported top-level function with a declared return type) and dispatches the final member on the factory return array's element component type, while overloaded factories, unknown factories, primitive- or multi-dimensional-returning factories, and non-array-returning factories fail closed, and a qualified base such as `val second = Util.makeItems()[0]` or `val third = group.makeGroups()[0]` resolves the leading call through the same qualified-initializer rules before dispatching on the element component type. A member chain after an element-access-inferred `val` such as `val first = makeItems()[0]; first.item.helper(...)` or `val first = makeItems()[0]; first.inner().helper(...)` starts on the resolved element component type and walks each trailing property or method-call hop through the same chain rules, including qualified field-chain bases such as `val first = group.fieldItems[0]; first.item.helper(...)` or multi-hop chains such as `val first = group.holder.fieldItems[0]; first.inner().helper(...)` that resolve each intermediate field's declared type before the terminal array field, while unknown method-call or property hops, primitive or multi-dimensional components, and unresolvable bases fail closed, and the same chains apply when the element-access-inferred `val` base resolves in an explicitly imported package with the hop types resolved in the declaring type's own package scope. The same `val` factory-call element-access binding applies when the factory is declared in an explicitly imported package, with the element component type resolved in the factory's own package scope, while overloaded factories, unresolvable qualified callees, unknown factories, and primitive-, multi-dimensional-, or non-array-returning imported factories still fail closed. A `val` local initialized from a factory call whose declared return type is a single-level array, such as `val items = makeItems()` with `fun makeItems(): Array<Helper>` or `val items = Util.makeItems()` with a companion-object, named-object, or bound-receiver callee, dispatches an element access on the array's element component type through the same factory rules, with qualified callees resolving through companion, explicit-companion (`Util.Companion.makeItems` or `Util.Factory.makeItems`), object, or bound-receiver member rules, while direct member calls on the array, unknown factories, non-array return types, constructor initializers, and unresolvable qualified callees fail closed. The same `val` factory-returned-array binding applies when the bare factory is declared in an explicitly imported package, with the element component type resolved in the factory's own package scope, while unknown factories, primitive- or multi-dimensional-returning factories, and non-array-returning imported factories still fail closed. A `val` local bound from a `super`-rooted element access such as `val first = super.inheritedItems[0]` where the direct superclass and the array property's element type are declared in an explicitly imported package dispatches the final member on the element component type resolved in the superclass's own package scope, while a direct superclass lacking the array property and unknown superclass array properties still fail closed. A `val` local bound from an element access with a companion-object or object root such as `val first = Util.fieldItems[0]` or `val second = Factory.fieldItems[0]` where the static root is declared in an explicitly imported package dispatches the final member on the static root's array property element component type resolved in the static root's own package scope, while unknown static roots, unknown static fields, and non-property static roots still fail closed. A deep object chain whose root and nested objects are declared in an explicitly imported package, such as `Outer.Inner.holder.member.run(...)`, resolves each intermediate property's declared type in the declaring type's own package scope before dispatching the terminal member, while unknown nested objects and unknown property hops still fail closed. Nullable declared spellings such as `Box?`, `Box<String>?`, and `Entry?` declared in an explicitly imported package normalize to the underlying raw base type for bound parameters, property hops, method-return hops, factory returns, and factory-inferred local bindings, so the trailing member dispatches on the same imported raw class declaration, while unknown hops, missing final members, unresolvable nullable factory return types, nullable generic array receivers, and nullable value-type receivers still fail closed. A property without an explicit declared type whose initializer is a bare-identifier constructor call such as `val inferred = Other()` pins the hop through the imported constructed type when the enclosing type is declared in an explicitly imported package, so the trailing member dispatches across packages, while missing properties and factory initializers without a declared return type still fail closed. An aliased import of an outer type such as `import org.util.Outer as O` resolves dotted nested constructors (`O.Inner()`), dotted nested companions (`O.Inner.Companion.helper(...)`), and dotted nested types in property chains (`O.Group().holder.member.helper(...)`) across packages, while unknown nested types and unknown property hops still fail closed. Parenthesized `val` initializers such as `val constructed = (Helper())`, `val items = (makeItems())`, or `val first = (group.fieldItems[0])` unwrap to the same receiver type or terminal array element component type as the unparenthesized form, including nested parentheses, while parenthesized primitive-array or multi-dimensional element accesses, `this`-rooted bases, unknown factories, and unknown qualified callees fail closed. Parenthesized receivers in member-chain calls such as `(group).entry.helper(...)`, `(group).inner().entry.helper(...)`, `(makeGroup()).entry.helper(...)`, `(Group()).entry.helper(...)`, or `((group)).entry.helper(...)` unwrap to the same chain spelling as the unparenthesized form and dispatch the trailing member or method-call hop on the same resolved receiver, while nullable parenthesized receivers such as `(group)?.entry.helper(...)` and unknown chain hops fail closed. Parenthesized member-chain receivers whose bound type, hop types, factory, and trailing member are declared in an explicitly imported package unwrap to the same chain spelling and resolve each hop's declared type in the declaring type's own package so the trailing member dispatches across packages, while unknown chain hops, unknown parenthesized roots, and nullable parenthesized receivers still fail closed. A `this`-rooted receiver such as `this.entry.helper(...)`, `this.helper(...)`, or `this.makeGroup(...)` dispatches on the enclosing type through the same member-chain rules as bound receivers, including members inherited through the enclosing type direct superclass chain and implemented-interface fallback, while unknown `this`-rooted hops and missing final members fail closed. A `super`-rooted receiver such as `super.entry.helper(...)` or `super.baseHelper(...)` dispatches on the direct superclass path through the same member-chain rules, walking the direct superclass chain to members declared on any ancestor class, while unknown `super`-rooted hops, missing final members, and classes without a resolvable superclass fail closed. Method-call hops inside member chains such as `group.inner().entry.helper(...)`, `this.makeInner().entry.helper(...)`, `super.inner().entry.helper(...)`, `makeGroup().inner().entry.helper(...)`, or `Group().inner().entry.helper(...)` dispatch a unique member or extension function with a declared return type (member functions shadow extensions, and ambiguous overload or extension sets fail closed) and continue the chain on the hop's declared return type, while unknown hops, primitive or unresolvable return types, and unbound receiver roots fail closed. A method-call hop on an interface-typed receiver such as `builder.inner().helper(...)` with `interface Builder { fun inner(): Entry }` dispatches a method directly declared on that interface from a bound parameter, a property hop that pins an interface type, or an imported interface in another file, while interface methods returning unknown or primitive types and unknown interface method hops fail closed. An interface-typed receiver also dispatches members, property hops, and method-call hops declared on a parent interface in its extends chain, resolving exactly one uniquely declared branch through any number of intermediate interfaces while competing declarations across branches, cyclic or unresolvable parent interfaces, and unknown or ambiguous receiver types fail closed. A class-typed receiver also dispatches a member declared on a parent class in its direct superclass chain when neither the class nor any nearer superclass declares it, resolving each supertype in the class's own file and package scope through any number of intermediate classes while nearer declarations shadow inherited ones and unknown members, ambiguous classes, cyclic or unresolvable superclass chains, and competing overload sets fail closed, and member-chain property and method-call hops on a class-typed receiver resolve through the same direct superclass chain and implemented-interface fallbacks before the extension fallback, so inherited hops such as `derived.entry.helper(...)` or `derived.inner().helper(...)` continue the chain on the inherited declared type, and cross-file class-receiver hierarchy hops such as `derived.entry.helper(...)` or `impl.inner().helper(...)` resolve inherited property and method-call hops declared in an explicitly imported package through the same superclass-chain and implemented-interface fallbacks with each hop's declared type resolved in the declaring type's own file and package scope before the trailing member dispatches while unresolvable imported supertypes and implemented interfaces fail closed for the affected hop, and cross-file `this.`- and `super.`-rooted class-receiver hierarchy hops such as `this.entry.helper(...)`, `this.inner().helper(...)`, `super.entry.helper(...)`, or `super.inner().helper(...)` dispatch the same imported superclass-chain and implemented-interface members and hops from inside the enclosing class while unresolvable imported supertypes and implemented interfaces fail closed for the affected root, and nullable `this.`- and `super.`-rooted hops such as `this.member.helper(...)` or `super.entry.helper(...)` where the hop type is declared nullable in the imported package normalize to the underlying imported declaration before dispatching the trailing member while unknown nullable hops fail closed, including hops reached through a diamond-shaped implemented-interface graph such as `class Impl : Diamond` with `interface Diamond : Left, Right`, `interface Left : Root`, and `interface Right : Root` resolving the shared-ancestor hop exactly once while competing or blocked diamond branches fail closed, and cross-file diamond-shaped implemented-interface chains such as `class Impl : Diamond` with `Diamond`, `Left`, `Right`, and `Root` declared in an explicitly imported package resolve the same shared-ancestor property and method-call hops exactly once through the imported interface chain while competing or blocked imported diamond branches fail closed, and a generic interface extends chain declared in an imported package dispatches property and method-call hops declared on the raw generic base with each hop type resolved in the declaring interface's own package while unresolvable hop types fail closed, and an aliased type import such as `import org.util.Base as B` with `class Derived : B()` or `class Impl : R` resolves the imported superclass and implemented interface through the alias binding for the same class-receiver hierarchy dispatch while unresolvable aliased supertypes and implemented interfaces fail closed, and nullable class-typed receiver spellings such as `Derived?` or `Impl?` normalize to their raw class before the same imported class-receiver hierarchy dispatch for property and method-call hops while unresolvable imported supertypes and implemented interfaces fail closed, and a constructed receiver from an imported package such as `Base().entry.helper(...)` or `Base().inner().helper(...)` dispatches property and method-call hops declared on an ancestor class in the imported package's own superclass chain with the hop type resolved in the declaring type's own package while unknown hops, unresolvable hop types, and blocked constructed superclass chains fail closed, and a deep constructor-rooted chain such as `Group().mid.inner.helper(...)` whose constructed type and each intermediate property's declared type are declared in an explicitly imported package resolves every hop in the declaring type's own package scope before dispatching the trailing member while unknown intermediate hops and unresolvable hop types fail closed, and element-access receivers on array-typed parameters, locals, and enclosing-class properties such as `items[0].helper(...)` dispatch on an imported element component type while unresolvable element types and non-array subscript bases fail closed, and member chains after an element hop such as `groups[0].item.helper(...)` or `groups[0].inner().helper(...)` walk each subsequent property or method-call hop through the same chain rules on the element type while unknown hops and unresolvable hop types fail closed, and an intermediate array-property element-access hop such as `groups[0]` in `holder.groups[0].item.helper(...)` or `holder.groups[0].inner().helper(...)` resolves the base name as a uniquely declared single-level array property on the current receiver and continues the chain on the element component type, including object-rooted receivers such as `Registry.groups[0].item.helper(...)`, named-companion receivers such as `Config.Factory.groups[0].item.helper(...)`, nested array-property hops such as `holder.groups[0].items[1].helper(...)`, and cross-file hops with the array property and element component type resolved in the imported package, while non-array properties, multi-dimensional arrays, primitive arrays, unknown properties, and direct member calls on the array itself fail closed, and a companion-chain root followed by an array-property element-access and a method-call hop such as `Config.Factory.groups[0].inner().helper(...)` or `Outer.Inner.NestedFactory.groups[0].inner().helper(...)` consumes the leading companion hops (`Config.Factory` consumes two, `Outer.Inner.NestedFactory` consumes three) onto the canonical companion scope, resolves the array-property element-access hop on that scope, and dispatches the trailing member on the method-call hop's declared return type, including cross-file chains whose companion is declared in an imported package, while unknown or non-array properties, unknown method-call hops, locally shadowed roots, and unknown roots fail closed, and a `this`-rooted or `super`-rooted factory element-access receiver such as `this.ownMake()[0].helper(...)` or `super.inheritedMake()[0].helper(...)` resolves the leading call as a factory declared on the enclosing type or walked through the direct superclass chain (including cross-file base classes resolved through explicit imports) before dispatching on the factory return array's element component type, with trailing property and method-call hops continuing on the element type such as `this.inheritedGroups()[0].inner().helper(...)`, while unknown factories, primitive-returning factories, multi-dimensional return arrays, and non-array-returning factories on `this`/`super` roots fail closed, and a `val` local bound from an element access on a companion-chain array property such as `val first = Config.Factory.groups[0]` or `val nested = Outer.Inner.NestedFactory.groups[0]` infers the base array's element component type from the canonical companion scope, so trailing property and method-call hops such as `first.item.helper(...)` or `first.inner().helper(...)` dispatch on the inferred type, including canonical `Config.Companion.groups[0]` spellings and cross-file bases whose companion is declared in an imported package, while unknown or non-array bases, element accesses directly on the companion object itself, locally shadowed roots, and unknown roots fail closed, and a factory-call element-access receiver followed by a member chain such as `makeGroups()[0].inner().helper(...)` or `makeGroups()[0].item.helper(...)` resolves the leading factory through the same factory rules as a direct factory-call element-access receiver and walks the trailing hops on the factory return array's element component type resolved in the factory's own package scope, including qualified callees through explicitly imported companion or object roots such as `Util.makeGroups()[0].inner().helper(...)`, bound-receiver callees such as `group.makeGroups()[0].item.helper(...)` with the receiver's declared type resolved in the imported package, and `val` locals bound from a qualified factory call such as `val items = Util.makeGroups(); items[0].inner().helper(...)`, while unknown or ambiguous factories, primitive or multi-dimensional return arrays, and unresolvable component or hop types fail closed, and companion property chains such as `Config.Factory.holder.run(...)` or `Config.Companion.holder.run(...)` where the companion is declared in an imported package resolve each intermediate property's declared type within the companion scope so the trailing member dispatches across packages while unresolvable companion property types and unknown companion chain hops fail closed, and a `val` local initialized from a qualified factory call such as `val items = Util.makeItems()` where the companion, object, or bound-receiver factory is declared in an explicitly imported package dispatches an element access on the factory return array's element component type resolved in the factory's own package scope while unresolvable component types and primitive array components fail closed, and a bare factory-call element-access receiver such as `makeItems()[0].helper(...)` with `makeItems` declared in an explicitly imported package dispatches the final member on the factory return array's element component type resolved in the factory's own package scope while unresolvable component types and primitive array components fail closed, while unknown hops fail closed, and a class-typed receiver whose class and direct superclass chain do not declare the method dispatches a uniquely arity-matched member declared on one of its directly implemented interfaces when exactly one direct-interface chain provides it and every other chain proves it has no declaration, where a member reached through multiple branches of the same diamond-shaped implemented-interface graph such as `class DiamondImpl : Diamond` with `interface Diamond : Left, Right`, `interface Left : Root`, and `interface Right : Root`, or through two direct interfaces that share a common ancestor declaration, still resolves exactly once, and generic implemented-interface spellings such as `class Impl : IBox<String>` (including cross-file spellings resolved through the explicit import in the class file such as `import org.util.IBox`, dispatching interface members and method-call hops declared in the imported package), generic superclass specifiers such as `class Derived : Base<Entry>()` (including cross-file spellings resolved through the explicit import in the class file such as `import org.util.Base`, dispatching `super.`-rooted member calls and `super.`- or `this.`-rooted property hops declared on the imported raw base with hop types resolved in the base's own package scope while unknown raw generic bases, missing superclass members, and unresolvable hop types fail closed), and nullable class-typed receiver spellings such as `Derived?` or `Impl?` normalize to their raw base types or class declarations for the same class-hierarchy dispatch while unresolvable raw generic bases fail closed, and same-name declarations in the receiver class hierarchy, competing or unresolved interface chains, and classes without implemented interfaces fail closed. A generic interface-typed receiver such as `IBox<String>` or a nullable generic interface spelling such as `IBox<String>?` normalizes to its raw interface before walking the same interface extends chain for members, property hops, and method-call hops, while competing generic branches, unresolvable generic parent interfaces, and unknown generic hops fail closed. A member reachable through multiple branches of the same diamond-shaped interface hierarchy, or through one branch that declares it while every other branch proves it absent, resolves exactly once, and property and method-call hops reached through the same diamond-shaped extends chain continue on the shared ancestor declared type exactly once, while competing declarations on different branches and any blocked (unresolvable or cyclic) branch fail closed. and a `val` local bound from an element access with a `this`-rooted base such as `val first = this.groups[0]` or `val first = this.inheritedGroups[0]` dispatches on the enclosing type's array property element component type, walking the direct superclass chain when the terminal array property is inherited from a parent class and resolving inherited terminal properties through the class or interface chain (including cross-file base classes resolved through explicit imports), with `this` inside a companion member dispatching on the companion object's array property, so trailing property and method-call hops such as `first.item.helper(...)` or `first.inner().helper(...)` continue on the inferred element type, while unknown, primitive-array, multi-dimensional-array, and non-array terminal properties on `this` roots and `this`-rooted bases in top-level functions fail closed and a `val` local initialized from a `this`- or `super`-rooted factory call such as `val items = this.ownMake()` or `val items = super.inheritedMake()` resolves the callee as a member function on the enclosing type (or the companion scope inside a companion member) or on the direct superclass chain, including inherited and cross-file factories resolved through explicit imports, and dispatches trailing element-access and method-call hops such as `items[0].inner().helper(...)` on the callee's declared single-level array return element component type, while a non-array `this`-rooted factory such as `val group = this.makeGroup()` pins the receiver to the declared return type, and unknown callees, primitive or multi-dimensional array returns, non-array returns on element-access chains, `this`/`super` roots outside a type, and ambiguous callees fail closed and a `val` local bound from a `this`- or `super`-rooted factory element-access base such as `val first = this.ownMake()[0]` or `val second = super.inheritedMake()[0]` resolves the callee as a member function on the enclosing type (or the companion scope inside a companion member) or on the direct superclass chain, including inherited and cross-file factories, and dispatches the terminal member or trailing method-call hops on the callee's declared single-level array return element component type, while unknown callees, primitive or multi-dimensional array returns, non-array returns, `this`/`super` roots outside a type, and ambiguous callees fail closed, and nullable single-level array factory returns such as `fun makeNullable(): Array<Helper>?` dispatch the same element component type whether the element access, the initializer, or the inline receiver carries a postfix `!!` force-unwrap such as `val items = this.makeNullable()!!`, `items!![0].helper(...)`, or `this.makeNullable()!![0].helper(...)`, including `super`-rooted and cross-file nullable factories resolved through explicit imports, while unknown nullable callees, nullable primitive or multi-dimensional array returns, nullable non-array returns, and `this`/`super` roots outside a type fail closed, and `val` locals, parameters, and `this`/`super`-rooted properties declared with a nullable single-level array type such as `val items: Array<Helper>?` or `fun run(items: Array<Helper>?)` dispatch a `!!` force-unwrapped element access on the declared element component type, including inherited and cross-file nullable array properties resolved through explicit imports, while nullable primitive arrays, multi-dimensional arrays, unresolvable components, and non-array nullable declared types fail closed, and an inline element-access chain rooted at a class name whose terminal array property lives on the class's anonymous companion object such as `Util.items[0].helper(...)` or `Util.groups!![0].inner().helper(...)` dispatches on the declared element component type (including nullable force-unwrapped spellings, trailing method-call hops, and cross-file companions resolved through explicit imports) while unknown properties, non-array properties, primitive and multi-dimensional arrays, classes without companion objects, unknown roots, and locally shadowed roots fail closed, and a bare `Companion` root inside a type such as `Companion.items[0].helper(...)` or `Companion.groups!![0].inner().helper(...)` dispatches on the enclosing type's canonical companion scope, resolving inline element-access chains, nullable force-unwrapped spellings, trailing method-call hops, `val` element-access initializers, and companion member calls such as `Companion.make().helper(...)` the same way as `this` inside a companion member (including from inside the companion object itself), while `Companion` roots in top-level functions, types without a companion object, unknown properties, non-array properties, primitive and multi-dimensional arrays, and locally shadowed roots fail closed, and a bare companion property reference inside a member function such as `items[0].helper(...)`, `nullableItems!![0].helper(...)`, `groups[0].inner().helper(...)`, or `val first = items[0]` dispatches on the companion property's declared single-level array element component type the same way as the `Companion.` spelling (including from inside the companion object itself) while a local, parameter, or instance property of the same name shadows the companion binding per Kotlin scope rules and non-array, primitive, multi-dimensional, unknown, no-companion, and top-level bare companion references fail closed, and a bare companion member function call inside a member function such as `make().helper(...)`, `makeItems()[0].helper(...)`, the nullable force-unwrapped spellings `makeNullableItems()!![0].helper(...)` or `makeNullableGroups()!![0].inner().helper(...)`, and `val` locals such as `val first = make()` or `val items = makeItems()` dispatches on the companion function's declared return type or single-level array return element component type the same way as the `Companion.` spelling (including from inside the companion object itself) while unknown functions, companion functions without a declared return type, return types without the trailing member, primitive returns, types without a companion object, and bare companion function calls in top-level functions fail closed, and a bare member function call inside a member function (an implicit `this` receiver) such as `make().helper(...)`, `makeItems()[0].helper(...)`, the nullable force-unwrapped spellings `makeNullableItems()!![0].helper(...)` or `makeNullableGroups()!![0].inner().helper(...)`, and `val` locals such as `val first = make()` or `val items = makeItems()` dispatches on the enclosing type's member function declared return type or single-level array return element component type the same way as the explicit `this.` spelling, with the enclosing type's member function shadowing a same-named companion member while unknown member functions, member functions without a declared return type, return types without the trailing member, and primitive returns fail closed, and a bare inherited property reference inside a member function (an implicit `this` receiver) such as `item.helper(...)`, `holder.item.helper(...)`, or `items[0].helper(...)` where the property is declared on a direct or transitive superclass dispatches through the same direct and inherited member-chain rules as the explicit `this.` spelling, with local variables and same-class properties shadowing the inherited property per Kotlin scope rules while unknown properties, inherited properties whose declared type lacks the trailing member or intermediate hop, properties without a usable declared type, and bare references in top-level functions fail closed, and a bare member function call inside a member function whose function is inherited from a direct or transitive superclass such as `helper(...)` inside a subclass of a base class that declares `fun helper(...)` dispatches through the same direct, inherited, and extension rules as the explicit `this.` spelling, with the enclosing type's own member function shadowing a same-named inherited function while arity mismatches, unknown functions, and bare calls in top-level functions fail closed, and a `val` local bound from a dotted property-chain initializer inside a member function such as `val first = holder.item`, `val first = this.holder.item`, or `val first = super.item` where the chain root or terminal property is declared on the enclosing type or a direct or transitive superclass dispatches its member calls through the terminal property type, with a body-local chain binding shadowing a same-named companion member while unknown chain roots, unknown intermediate hops, chains whose terminal property has no usable member dispatch, and chains in top-level functions fail closed, and a `val` local bound from a dotted property-chain initializer whose terminal property is a single-level array such as `val first = holder.items` or `val first = this.holder.items` dispatches an element access such as `first[0].helper(...)` or a trailing method-call chain such as `first[0].inner().helper(...)` on the terminal array's element component type, including inherited terminal array properties and `super`-rooted chains, with a body-local chain binding shadowing a same-named companion member while non-array terminals, unknown chain roots and intermediate hops, direct member calls on the array, and chains in top-level functions fail closed, and a `val` local bound from a bare property initializer inside a member function such as `val first = item` or `val first = items` where the property is declared on the enclosing type or a direct or transitive superclass records the property name as a single-hop property chain, dispatching member calls such as `first.helper(...)` and terminal array element access such as `first[0].helper(...)` or `first[0].inner().helper(...)` through the same own-and-inherited property rules, with a body-local binding shadowing a same-named companion member while unknown properties, element access on non-array properties, properties whose declared type lacks the trailing member or intermediate hop, and bare properties in top-level functions fail closed, and a `val` local bound from a property-chain initializer containing a method-call hop inside a member function such as `val first = make().item`, `val first = this.make().item`, `val first = super.make().item`, or `val first = this.holder.make().item` where the method-call hop is declared on the enclosing type or a direct or transitive superclass walks the hop's declared return type and the remaining property hops before dispatching member calls such as `first.helper(...)` on the terminal property type, with a body-local chain binding shadowing a same-named companion member while non-zero-argument call spellings, unknown member functions or terminal properties, chains whose terminal property has no usable member dispatch, and chains in top-level functions fail closed, and a `val` local bound from a property-chain initializer whose leading method-call hop is declared across files or as a top-level function inside a member function or top-level caller such as `val first = make().item` with `make` inherited from a `Base` declared in another file, `val first = samePackageHolder().item`, or `val first = makeHolder().item` with `makeHolder` a same-file, same-package, or explicitly imported top-level function dispatches through the callee's declared return type and the remaining property hops, including terminal single-level arrays such as `val first = makeHolder().items` with `first[0].helper(...)`, while unknown or ambiguous callees, callees whose declared return type is primitive or unresolvable, and chains whose terminal property has no usable member dispatch fail closed, and a `val` local bound from a property-chain initializer whose leading hop is a plain constructor call such as `val first = Holder().item` or `val first = Group().holder.item` starts the chain on the constructed type and walks the remaining property hops, including imported constructors such as `val first = Holder().item` with `Holder` explicitly imported from another package, while unknown constructed types, roots whose terminal property has no usable member dispatch, and generic-array constructor roots fail closed, and nullable and generic spellings normalize to their raw dotted base types for property-chain initializer roots, so `val first = makeNullable().item` with `fun makeNullable(): Holder?`, `val first = makeBox().item` with `fun makeBox(): Box<Holder>`, and `val first = Box<Holder>().item` all dispatch through the raw `Holder` or `Box` type's property, while safe-call roots such as `Holder()?.item`, generic-array constructor roots such as `Array<Item>().item`, and generic roots whose raw base lacks the terminal property fail closed, and a `val` local bound from a property-chain initializer whose leading bare hop names a named object such as `val first = Registry.holder.item`, a class with an explicit or named companion chain such as `val first = AnonymousHost.Companion.holder.item` or `val first = NamedHost.Factory.holder.item`, or a class with an anonymous companion such as `val first = AnonymousHost.holder.item` starts the chain on that object or companion scope and walks the remaining property hops, including terminal single-level arrays such as `val first = Registry.items` with `first[0].helper(...)`, while classes without a companion, unknown objects, companion scopes that lack the terminal property, and a `val` local bound from an earlier statement such as `val x = Holder()` starts a later property-chain initializer such as `val first = x.item` on the bound value's declared type, its recorded property-chain terminal type (such as `val x = group.holder`), or its element-access component type (such as `val x = items[0]` or `val x = makeItems()[0]`), shadowing a same-named object root, while bound names whose declared type or base cannot resolve fail closed, and a bare first hop that names a same-package or explicitly imported top-level property such as `val first = holder.item` with `val holder: Holder = Holder()` at package scope starts the chain on the property's declared type while an own or inherited property of the enclosing type shadows a same-named top-level property and unknown, ambiguous, primitive, and untyped top-level properties fail closed. A `val` local bound from a property-chain initializer whose leading hop is a same-package or explicitly imported top-level array property such as `val first = itemGroup[0]` or `val first = itemGroup` with `val itemGroup: Array<Holder>` at package scope dispatches element access such as `first[0].helper(...)` on the array's element component type while a locally bound or member name shadows the top-level property and element access on a bound non-array base fails closed, and a `val` local bound from a property-chain initializer whose single hop names a bound value declared as or derived from a single-level array such as `val first = items` with `items` an own or inherited member property, parameter, or explicitly typed local, or re-bound from another chain or element-access base such as `val group = itemGroup` then `val first = group`, dispatches `first[0].helper(...)` on the element component type while bound non-array values fail closed, and a `val` local bound from a factory or method-call initializer whose declared return type is a single-level array such as `val x = makeItems()`, `val x = holder.make()`, `val x = h.maker.make()`, or `val x = Registry.factory.make()` dispatches `x[0].helper(...)` on the return array's element component type while factories returning non-arrays fail closed, and a `val` local bound from a property-chain initializer containing single-level element-access hops such as `val x = h.items[0].make()`, `val first = h.inners[0].item`, `val x = memberItems[0].make()`, or `val first = h.inners[0].inner()` walks each element-access hop through the array property's element component type and each terminal method-call hop through its declared return array's element component type before dispatching member calls such as `x[0].helper(...)` or `first.helper(...)`, including the direct inline form `h.items[0].make()[0].helper(...)`, while non-zero-argument call hops, multi-dimensional element access, element access on bound non-array values, and unresolvable hops fail closed. and a `val` local bound from a property-chain initializer whose leading single-level element-access hop indexes a same-package or explicitly imported top-level array property such as `val x = itemGroup[0].make()` with `val itemGroup: Array<Maker>` at package scope dispatches the terminal factory hop's declared return array element component type so `x[0].helper(...)` traces, while an own or inherited member array of the enclosing type shadows a same-named top-level property (still tracing through the member) and an own or inherited non-array member shadows the top-level property and fails closed, and unknown bases and unresolvable components fail closed. It still makes no Java/JVM source-linkage assumptions and does not advertise patch operations.
