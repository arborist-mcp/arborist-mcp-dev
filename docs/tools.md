...
- Go: `.go` — Tree-sitter parsing, raw queries, semantic skeletons, and conservative declaration
  ...
  `Counter{}.Value()` to one matching production method in the same local package, or through an unshadowed named local receiver or local-type parameter. Module-root imports, external
