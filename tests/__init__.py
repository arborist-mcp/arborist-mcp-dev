...
from tests.gateway_protocol import GROUPS as GATEWAY_GROUPS
...
def _load_suites(manifest: dict[str, object]) -> dict[str, dict[str, object]]:
    ...
    if not isinstance(raw, dict):
        raise RuntimeError("python test suite manifest 'suites' must be a JSON object")

    suites: dict[str, dict[str, object]] = {}
    for suite_name, metadata in raw.items():
        if not isinstance(suite_name, str) or not suite_name.strip():
            raise RuntimeError("python test suite names must be non-empty strings")
        if not isinstance(metadata, dict):
            raise RuntimeError(f"python test suite '{suite_name}' metadata must be a JSON object")

        module_name = metadata.get("module")
        if not isinstance(module_name, str) or not module_name.strip():
            raise RuntimeError(f"python test suite '{suite_name}' must define a non-empty module name")

        description = metadata.get("description")
        if not isinstance(description, str) or not description.strip():
            raise RuntimeError(f"python test suite '{suite_name}' must define a non-empty description")

        requires_extension = metadata.get("requires_extension")
        if not isinstance(requires_extension, bool):
            raise RuntimeError(
                f"python test suite '{suite_name}' must define a boolean requires_extension flag"
            )
...
def _load_groups(manifest: dict[str, object]) -> dict[str, dict[str, object]]:
    raw = manifest["groups"]
    if not isinstance(raw, dict):
        raise RuntimeError("python test suite manifest 'groups' must be a JSON object")

    groups: dict[str, dict[str, object]] = {}
    for group_name, metadata in raw.items():
        if not isinstance(group_name, str) or not group_name.strip():
            raise RuntimeError("python test group names must be non-empty strings")
        if not isinstance(metadata, dict):
            raise RuntimeError(f"python test group '{group_name}' metadata must be a JSON object")

        description = metadata.get("description")
        if not isinstance(description, str) or not description.strip():
            raise RuntimeError(f"python test group '{group_name}' must define a non-empty description")

        entries = metadata.get("entries")
        if not isinstance(entries, list) or not entries:
            raise RuntimeError(
                f"python test group '{group_name}' must define a non-empty entries list"
            )
        ...
        for entry in entries:
            if not isinstance(entry, str) or not entry.strip():
                raise RuntimeError(
                    f"python test group '{group_name}' entries must be non-empty strings"
                )
...
def _merge_suites(
    local_suites: dict[str, dict[str, object]],
    gateway_suites: dict[str, dict[str, object]],
) -> dict[str, dict[str, object]]:
    duplicates = set(local_suites) & set(gateway_suites)
    if duplicates:
        raise RuntimeError(f"duplicate python test suite names: {sorted(duplicates)}")
    return {**local_suites, **gateway_suites}


def _merge_groups(
    local_groups: dict[str, dict[str, object]],
    gateway_groups: dict[str, dict[str, object]],
) -> dict[str, dict[str, object]]:
    duplicates = set(local_groups) & set(gateway_groups)
    if duplicates:
        raise RuntimeError(f"duplicate python test group names: {sorted(duplicates)}")
    return {**local_groups, **gateway_groups}
...
def _expand_group(
    ...
    *,
    ...
    if group_name not in groups:
        raise RuntimeError(f"unknown python test group '{group_name}'")
    if group_name in stack:
        cycle = " -> ".join((*stack, group_name))
        raise RuntimeError(f"python test group cycle detected: {cycle}")
...
_LOCAL_SUITES = _load_suites(MANIFEST)
_LOCAL_GROUPS = _load_groups(MANIFEST)
SUITES = _merge_suites(_LOCAL_SUITES, GATEWAY_SUITES)
_GROUP_DEFINITIONS = _merge_groups(_LOCAL_GROUPS, GATEWAY_GROUPS)
SUITE_MODULES = {
...
MODULE_NAMES = GROUP_MODULES["python"]
