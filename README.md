...
- Kotlin: `.kt`, `.kts` — Tree-sitter parsing, raw query execution, and package-qualified semantic skeletons for top-level and nested classes, interfaces, enums, named objects, functions, simple properties, and type aliases. Local declarations inside function bodies are intentionally omitted because they have no stable file-level semantic path. Kotlin currently makes no Java/JVM source-linkage assumptions and does not advertise declaration indexing, dependency refresh, tracing, or patch operations.
