...
- JavaScript: `.js`, `.jsx`, `.mjs`, `.cjs` — semantic skeletons, indexing,
  ...
  default-import (including CommonJS `module.exports = <class>` exports),
  namespace-member, and CommonJS class/function exports,
  `require` bindings (including destructured members with default values) and TypeScript `import name = require(...)` bindings, object-literal and member-assignment export members (including named function/class expression values and export-object `default` entries),
  module-valued export members that alias another module's export object or a named member,
  destructured `require` and named-import bindings through those module-valued export members,
  ...
  queries. Patch binding validation resolves identifier references inside a patched symbol to visible parameters (including destructured, default, rest, and TypeScript parameter-property bindings), local `const`/`let`/`var` declarators including destructured declarations, destructuring assignment targets, loop variables, catch parameters, nested callable parameters, decorator expressions, same-file top-level declarations, or explicit named, default, and namespace imports. Type-only imports, local interfaces and type aliases, const enums, and import aliases to type-only declarations cannot satisfy runtime value references; member names, object keys, labels, JSX tag and attribute names, type spellings, and standard host/global names are ignored, and unknown bare identifiers fail closed.
...
- Go: `.go` — Tree-sitter parsing, raw queries, semantic skeletons, and conservative declaration
  ...
  production method in the same local package; a direct named type-conversion receiver such as `Scalar(value).Value()`, `(*Scalar)(value).Value()`, `(Scalar)(value).Value()`, or `Box[int](value).Value()` when its base type is one unique same-package production `type` specification; a direct named type-assertion receiver such as `value.(Scalar).Value()` when `Scalar` is one unique same-package production `type` specification; a simple local alias receiver such as `type Alias = Counter; Alias{}.Value()`, `Alias(value).Value()`, or `value.(Alias).Value()` when its alias chain reaches one unique same-package production named `type` declaration without a cycle; or an unshadowed named local receiver, local-type parameter, or directly declared function-body local variable; a direct method call on a same-file factory result whose unique return type is a directly declared local interface, such as `NewWorker().Run(1)`, resolves to that interface's unique method specification while preserving the factory call edge; a direct method call through a local interface that directly embeds one uniquely resolved same-package interface, such as `type Worker interface { Base }`, resolves to the embedded interface's unique method specification. If the conversion-shaped receiver name has no matching local type specification, it retains a direct factory-function dependency rather than guessing a method target. Same-package production sources refresh conservatively as a group. Module-root imports, external modules,
...
- C#: `.cs` — Tree-sitter parsing, raw queries, semantic skeletons, declaration indexing, and
  ...
  unchanged C# source files are not reindexed; a refreshed source that has changed may safely shrink because its stale byte ranges do not block validation before the rebuilt index replaces them. Structural patch targeting covers classes, structs, interfaces, enums, records, methods, and constructors by semantic path or source position with syntax-level validation; a further slice adds C# patch binding validation: identifier
  references inside a patched symbol resolve to visible formal parameters, local declarators, `for` and
  `foreach` variables, catch parameters, using-resource variables, lambda and anonymous-method parameters,
  local functions and their parameters, pattern variables, out-variables, query variables, record components,
  same-file type, method, field, property, event, and enum-constant declarations, or aliased using directives
  declared in the caller's namespace, an enclosing namespace, or file root, with a unique nearest alias taking
  precedence. Type spellings, member names, labels, attribute names, cast/is/as/object-creation types,
  `nameof`/`typeof`/`sizeof` types, wildcard-like namespace and static imports, and standard C# names are
  ignored or fail closed, and unknown bare identifiers fail closed. Other member dispatch and overload type
  selection remain explicitly unavailable until dedicated C# adapter slices establish their contracts and fixtures.
...
- Rust Tree-sitter parsing, raw query execution, semantic skeletons, declaration indexing, and
  ...
  top-level function or through a crate-root `pub use` re-export to the defining module. Exact
  ...
  same impl type, while trait-impl methods, nested functions, `Self` used outside an impl, and missing
  or deeper unresolvable static targets fail closed.
  ...
  Member-chain instance calls through zero-argument method-call hops such as `outer.get_inner().increment();` or `make_root().middle().leaf().run();` resolve each intermediate call's declared return type when it is a plain, unimported, same-file struct name returned by an inherent `impl` method with only `self` parameters or by a zero-argument top-level function, resolved in the callable's own module scope, before dispatching the final method, while argument-taking calls, generic, primitive, referenced, path-typed, or missing return types, unknown methods or functions, and unknown base receivers fail closed. Trailing turbofish generic segments on member-chain call hops such as `outer.get_inner::<u8>().increment();` or `make_root::<u8>().go();`, including their `let` bindings such as `let inner = outer.get_inner::<u8>(); inner.increment();`, are ignored for resolution, so they trace to the same intermediate call and final method as the corresponding plain hops, while unknown methods, functions, or return types still fail closed.
  Member-chain instance calls whose return value is bound with `let`, such as `let inner = outer.get_inner(); inner.increment();` or `let root = make_root(); root.middle().leaf().run();`, infer each bound local's declared plain struct type from the same zero-argument call-hop rules and then dispatch the trailing member chain on it, while argument-taking calls, generic, primitive, referenced, path-typed, or missing return types, unknown methods or functions, shadowed function bindings, and unknown base receivers fail closed. Field-access `let` bindings such as `let inner = outer.inner; inner.increment();`, `let leaf = root.middle.leaf; leaf.run();`, `let inner = Outer { inner: Inner {} }.inner; inner.increment();`, or `let inner = self.inner; inner.increment();` infer each bound local's declared plain struct field type from the same field-chain resolution and then dispatch the trailing method on it, while unknown fields, unknown base receivers, generic or non-plain field types, and missing methods fail closed.
  Member-chain instance calls whose return value is bound with `let`, such as `let inner = outer.get_inner(); inner.increment();` or `let root = make_root(); root.middle().leaf().run();`, infer each bound local's declared plain struct type from the same zero-argument call-hop rules and then dispatch the trailing member chain on it, while argument-taking calls, generic, primitive, referenced, path-typed, or missing return types, unknown methods or functions, shadowed function bindings, and unknown base receivers fail closed.
...
- Go Tree-sitter parsing, raw query execution, semantic skeletons, and conservative declaration
  ...
  named composite literal such as `Counter{}.Value()`, `(&Counter{}).Value()`, or `Box[int]{}.Value()` to one matching production method in the same local package; a direct named type-conversion receiver such as `Scalar(value).Value()`, `(*Scalar)(value).Value()`, `(Scalar)(value).Value()`, or `Box[int](value).Value()` when its base type is one unique same-package production `type` specification; a direct named type-assertion receiver such as `value.(Scalar).Value()` when `Scalar` is one unique same-package production `type` specification; a simple local alias receiver such as `type Alias = Counter; Alias{}.Value()`, `Alias(value).Value()`, or `value.(Alias).Value()` when its alias chain reaches one unique same-package production named `type` declaration without a cycle; or an unshadowed named local receiver, local-type parameter, or directly declared function-body local variable. A direct method call on a generic local factory instantiation such as `NewWorker[int]().Run(1)`, argument-bearing instantiations such as `NewWorker[int](1).Run(2)`, and `var worker Worker = NewWorker[string](3)` initializers resolves through the factory's unique single-result interface return like the plain form, and the imported form `svc.NewWorker[int]().Run(1)` follows the same rules as its non-generic spelling when the import binding, unique exported factory, and unique interface method all resolve, including argument-bearing imported instantiations such as `svc.NewWorker[int](1).Run(2)`; ambiguous or unresolvable shapes fail closed while retaining only the factory dependency. If the conversion-shaped receiver name has no matching local type specification, it retains a direct factory-function dependency rather than guessing a method target. Same-package production sources refresh conservatively as a group. Module-root imports, external modules, `replace`, `go.work`, vendoring, build tags,
...
- C# Tree-sitter parsing, raw query execution, semantic skeletons, declaration indexing, and
  ...
  global-using files; unchanged C# source files are not reindexed. Structural patch targeting covers classes, structs, interfaces, enums, records, methods, and constructors by semantic path or source position with syntax-level validation; a further slice adds C# patch binding validation: identifier references
  inside a patched symbol resolve to visible formal parameters, local declarators, `for` and `foreach`
  variables, catch parameters, using-resource variables, lambda and anonymous-method parameters, local
  functions and their parameters, pattern variables, out-variables, query variables, record components,
  same-file type, method, field, property, event, and enum-constant declarations, or aliased using directives,
  while type spellings, member names, labels, attribute names, cast/is/as/object-creation types,
  `nameof`/`typeof`/`sizeof` types, wildcard-like namespace and static imports, and standard C# names are
  ignored or fail closed, and unknown bare identifiers fail closed. Other member dispatch and overload type
  selection remain capability-gated pending dedicated C# adapter slices.
...
- Extending C# member dispatch and overload type selection beyond conservative same-type, `base.`, static,
  alias/import, and global-using bindings, and extending C# patch binding validation beyond local, same-file,
