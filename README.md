...
- Go Tree-sitter parsing, raw query execution, semantic skeletons, and conservative declaration
  ...
  named composite literal such as `Counter{}.Value()` to one matching production method in the same local package, or through an unshadowed named local receiver, local-type parameter, or a directly declared function-body local variable. Same-package production sources refresh conservatively as a group. Module-root imports, external modules, `replace`, `go.work`, vendoring, build tags,
