...
- C#: `.cs` — Tree-sitter parsing, raw queries, semantic skeletons, declaration indexing, and
  ...
  `global using` aliases, dependency refresh, other member dispatch, overload type selection, and patching remain explicitly
...
- C# Tree-sitter parsing, raw query execution, semantic skeletons, declaration indexing, and
  ...
  (including directive-only files), root-level global namespace-import `Type.Method()` calls from any scanned source file,
  ...
  alias/import inheritance, `global using` aliases, dependency refresh, other member dispatch, overload type selection, and
