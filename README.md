...
- C# Tree-sitter parsing, raw query execution, semantic skeletons, declaration indexing, and
  ...
  unqualified/`this.` forms remain same-file. `base(...)` and `base.Method()` require one unique class/record base declaration. Simple and `global::` base names, plus unique unshadowed aliases declared at file root or in the caller's exact namespace, are supported. Constructor targets must have an exact-arity, non-`params` match; `base.Method()` targets must be directly declared non-static methods with one exact-arity, non-`params` match. Generic, non-`global::` qualified, global-using alias, ambiguous/colliding alias, and namespace-import base types fail closed. Imported and qualified static targets must be unique, static,
