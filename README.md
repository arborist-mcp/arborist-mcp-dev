...
- C#: `.cs` — Tree-sitter parsing, raw queries, semantic skeletons, declaration indexing, and
  ...
  and conservative `base(...)` constructor initializers and direct `base.Method()` calls with simple or `global::` base types,
...
- C# Tree-sitter parsing, raw query execution, semantic skeletons, declaration indexing, and
  ...
  unqualified/`this.` forms remain same-file. `base(...)` and `base.Method()` require one unique class/record base declaration. Constructor targets must have an exact-arity, non-`params` match; `base.Method()` targets must be directly declared non-static methods with one exact-arity, non-`params` match. Generic, non-`global::` qualified, alias, and namespace-import base types fail closed. Imported and qualified static targets must be unique, static,
