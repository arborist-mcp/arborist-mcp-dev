...
pub(crate) fn build_shell_skeleton(
    path: &Path,
    source: &str,
    tree: &Tree,
    depth_limit: usize,
    ...
    for node in collect_shell_symbol_nodes(tree.root_node(), deadline)? {
        if let Some(deadline) = deadline {
            deadline.check("collecting shell semantic symbols")?;
        }
        ...
        let signature = shell_signature(node, source).ok_or_else(|| {
            anyhow!("shell semantic symbol `{semantic_path}` has an empty signature")
        })?;
    ...
    for selector in &expand_set {
        ...
        {
            bail!(
                "ambiguous shell semantic path `{selector}`; duplicate declarations cannot be expanded safely"
            );
        }
    }
    ...
    for (node, semantic_path, symbol_id) in symbol_items {
        if let Some(deadline) = deadline {
            deadline.check("rendering shell semantic skeleton")?;
        }
        if expanded_items
            .iter()
        ...
        if expand_set.contains(semantic_path.as_str()) || expand_set.contains(symbol_id.as_str()) {
            skeleton_items.push(shell_full_declaration(node, source)?);
    ...
    if let Some(deadline) = deadline {
        deadline.check("validating shell semantic skeleton")?;
    }
...
pub(crate) fn find_shell_semantic_node<'tree>(
    path: &Path,
    tree: &'tree Tree,
    source: &str,
    target_path: &str,
    ...
    for node in collect_shell_symbol_nodes(tree.root_node(), deadline)? {
        if let Some(deadline) = deadline {
            deadline.check("resolving shell semantic target")?;
        }
        let Some(name) = shell_symbol_name(node, source)? else {
            continue;
        };
        if shell_semantic_path(name.as_str())?.as_deref() == Some(local_target) {
            matches.push(node);
        }
    }
    ...
    match matches.as_slice() {
        ...
        [node] => Ok(Some(*node)),
        _ => bail!(
            "ambiguous shell semantic path `{target_path}`; duplicate declarations cannot be resolved safely"
        ),
    }
}

pub(crate) fn is_shell_symbol_node(node: Node<'_>) -> bool {
    matches!(node.kind(), "function_definition")
}

pub(crate) fn shell_symbol_name(node: Node<'_>, source: &str) -> Result<Option<String>> {
...
pub(crate) fn shell_semantic_path(name: &str) -> Result<Option<String>> {
    if name.is_empty() {
        return Ok(None);
    }
...
pub(crate) fn shell_signature(node: Node<'_>, source: &str) -> Option<String> {
    let end_byte = node
...
pub(crate) fn shell_parameters(_node: Node<'_>, _source: &str) -> Vec<String> {
    Vec::new()
}

fn shell_full_declaration(node: Node<'_>, source: &str) -> Result<String> {
    let declaration = node_text(node, source)?.trim();
    if declaration.is_empty() {
        bail!("shell semantic declaration has empty source text");
    }
    Ok(declaration.to_string())
}

fn collect_shell_symbol_nodes<'tree>(
    root: Node<'tree>,
    deadline: Option<&dyn DeadlineCheck>,
...
mod tests {
    ...
    use super::{build_shell_skeleton, find_shell_semantic_node};
    use crate::LanguageId;
    use crate::language::parse_document;

    #[test]
    fn builds_shell_skeleton_for_function_definitions() {
...
"#;
        ...
        assert_eq!(document.language_id, LanguageId::Shell);
        let skeleton = build_shell_skeleton(path, source, &document.tree, 10, &[], None).unwrap();
        ...
        let compute_symbol = skeleton
            ...
            .unwrap();
        assert!(compute_symbol.parameters.is_empty());
        let found = find_shell_semantic_node(path, &document.tree, source, "compute", None)
            .unwrap()
            .unwrap();
