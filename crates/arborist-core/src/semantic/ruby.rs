...
pub(crate) fn build_ruby_skeleton(
    path: &Path,
    source: &str,
    tree: &Tree,
    depth_limit: usize,
    ...
    for node in collect_ruby_symbol_nodes(tree.root_node(), deadline)? {
        if let Some(deadline) = deadline {
            deadline.check("collecting Ruby semantic symbols")?;
        }
        ...
        let signature = ruby_signature(node, source).ok_or_else(|| {
            anyhow!("Ruby semantic symbol `{semantic_path}` has an empty signature")
        })?;
    ...
    for selector in &expand_set {
        ...
        {
            bail!(
                "ambiguous Ruby semantic path `{selector}`; duplicate declarations cannot be expanded safely"
            );
        }
    }
    ...
    for (node, semantic_path, symbol_id) in symbol_items {
        if let Some(deadline) = deadline {
            deadline.check("rendering Ruby semantic skeleton")?;
        }
        if expanded_items
            .iter()
        ...
        if expand_set.contains(semantic_path.as_str()) || expand_set.contains(symbol_id.as_str()) {
            skeleton_items.push(ruby_full_declaration(node, source)?);
    ...
    if let Some(deadline) = deadline {
        deadline.check("validating Ruby semantic skeleton")?;
    }
...
pub(crate) fn find_ruby_semantic_node<'tree>(
    path: &Path,
    tree: &'tree Tree,
    source: &str,
    target_path: &str,
    ...
    for node in collect_ruby_symbol_nodes(tree.root_node(), deadline)? {
        if let Some(deadline) = deadline {
            deadline.check("resolving Ruby semantic target")?;
        }
        let Some(name) = ruby_symbol_name(node, source)? else {
            continue;
        };
        if ruby_semantic_path(name.as_str())?.as_deref() == Some(local_target) {
            matches.push(node);
        }
    }
    ...
    match matches.as_slice() {
        ...
        [node] => Ok(Some(*node)),
        _ => bail!(
            "ambiguous Ruby semantic path `{target_path}`; duplicate declarations cannot be resolved safely"
        ),
    }
}

pub(crate) fn is_ruby_symbol_node(node: Node<'_>) -> bool {
    matches!(node.kind(), "method")
}

pub(crate) fn ruby_symbol_name(node: Node<'_>, source: &str) -> Result<Option<String>> {
...
pub(crate) fn ruby_semantic_path(name: &str) -> Result<Option<String>> {
    if name.is_empty() {
        return Ok(None);
    }
...
pub(crate) fn ruby_signature(node: Node<'_>, source: &str) -> Option<String> {
    let end_byte = node
...
pub(crate) fn ruby_parameters(node: Node<'_>, source: &str) -> Vec<String> {
...
fn ruby_full_declaration(node: Node<'_>, source: &str) -> Result<String> {
    let declaration = node_text(node, source)?.trim();
    if declaration.is_empty() {
        bail!("Ruby semantic declaration has empty source text");
    }
    Ok(declaration.to_string())
}

fn collect_ruby_symbol_nodes<'tree>(
    root: Node<'tree>,
    deadline: Option<&dyn DeadlineCheck>,
...
mod tests {
    ...
    use super::{build_ruby_skeleton, find_ruby_semantic_node};
    use crate::LanguageId;
    use crate::language::parse_document;

    #[test]
    fn builds_ruby_skeleton_for_method_definitions() {
...
";
        ...
        let skeleton = build_ruby_skeleton(path, source, &document.tree, 10, &[], None).unwrap();
        ...
        let found = find_ruby_semantic_node(path, &document.tree, source, "compute", None)
            .unwrap()
            .unwrap();
