...
pub use model::{
    ...
    SymbolIndexStats, SymbolListContextResult, SymbolListNeighborhoodContextResult,
    SymbolListResult, SymbolMeta, SymbolNeighborhoodContextResult, SymbolReadResult,
    SymbolSearchContextResult, SymbolSearchMatchDetail, SymbolSearchNeighborhoodContextResult,
...
pub use symbols::{
    ...
    search_symbols_context_from_index_filtered, search_symbols_filtered, search_symbols_from_index,
    search_symbols_from_index_filtered, search_symbols_neighborhood_context,
    search_symbols_neighborhood_context_filtered, search_symbols_neighborhood_context_from_index,
...
fn validate_neighborhood_context_patch_result(
    result: &NeighborhoodContextPatchResult,
) -> Result<()> {
    result.validate_public_output()?;
    if !result.patch.validation.syntax_errors.is_empty() || !result.patch.applied {
...
def orchestrate(value: int) -> int:\n    return value\n",
    ...
    fn lists_symbol_neighborhood_context_in_live_workspace_and_persisted_index() {
        ...
        let live =
            list_symbols_neighborhood_context(&dir, 10, TraceDirection::Callers, 2, 10).unwrap();
        ...
        rebuild_symbol_index(&dir, &db_path).unwrap();
        let persisted = list_symbols_neighborhood_context_from_index(
            &db_path,
            10,
            TraceDirection::Callers,
            2,
    ...
    fn reads_symbol_neighborhood_context_in_live_workspace_and_persisted_index() {
        ...
        let live = read_symbol_neighborhood_context(&dir, "helper", TraceDirection::Callers, 2, 10)
            .unwrap();
    ...
    fn traces_symbol_neighborhood_in_live_workspace_and_persisted_index() {
        ...
        let live =
            trace_symbol_neighborhood(&dir, "helper", TraceDirection::Callers, 2, 10).unwrap();
        ...
        rebuild_symbol_index(&dir, &db_path).unwrap();
        let persisted = trace_symbol_neighborhood_from_index(
            &db_path,
            "helper",
            TraceDirection::Callers,
            2,
    ...
    fn create_symbol_index_schema_with_text_byte_columns(connection: &Connection) {
        connection
            .execute_batch(
                "
                CREATE TABLE metadata (
