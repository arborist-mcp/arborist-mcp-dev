...
fn traces_go_generic_factory_initialized_local_receivers() {
    ...
    let overlay = "package metrics\n\ntype Worker interface { Run(value int) error }\nfunc NewWorker[T any]() Worker { return nil }\nfunc overlayCaller() error { worker := NewWorker[string](); return worker.Run(1) }\n";
...
#[test]
fn traces_go_imported_interface_factory_returns() {
    let dir = temporary_dir();
    let caller_path = dir.join("cmd").join("main.go");
    let service_path = dir.join("internal").join("service").join("service.go");
    let db_path = dir.join("symbols.db");
    ...
    let overlay = "package main\n\nimport svc \"example.com/project/internal/service\"\n\nfunc overlayCaller() error { return svc.NewWorker().Run(1) }\n";
    let dirty = trace_symbol_graph_with_source(
        &dir,
        &caller_path,
        overlay,
...
#[test]
fn traces_go_parenthesized_imported_interface_factory_returns() {
    let dir = temporary_dir();
    let caller_path = dir.join("cmd").join("main.go");
    let service_path = dir.join("internal").join("service").join("service.go");
    let db_path = dir.join("symbols.db");
...
#[test]
fn traces_go_generic_imported_interface_factory_returns() {
    let dir = temporary_dir();
    let caller_path = dir.join("cmd").join("main.go");
    let service_path = dir.join("internal").join("service").join("service.go");
    let db_path = dir.join("symbols.db");
    ...
    fs::write(
        &caller_path,
        "package main\n\nimport svc \"example.com/project/internal/service\"\n\nfunc caller() error { return svc.NewWorker[int]().Run(1) }\n",
    )
    .unwrap();
    fs::write(
        &service_path,
...
#[test]
fn traces_go_parenthesized_generic_local_interface_factory_returns() {
    let dir = temporary_dir();
    let source_path = dir.join("metrics.go");
    let db_path = dir.join("symbols.db");
    fs::write(
...
#[test]
fn traces_go_parenthesized_generic_imported_interface_factory_returns() {
    let dir = temporary_dir();
    let caller_path = dir.join("cmd").join("main.go");
    let service_path = dir.join("internal").join("service").join("service.go");
    let db_path = dir.join("symbols.db");
...
fn traces_go_imported_interface_factory_returns_with_declared_package_name() {
    ...
    fs::write(
        &caller_path,
        "package main\n\nimport \"example.com/project/internal/service\"\n\nfunc caller() error { return service.NewWorker().Run(1) }\n",
    )
    .unwrap();
    fs::write(
        &service_path,
...
#[test]
fn traces_go_imported_interface_factory_returns_from_direct_interface_aliases() {
    let dir = temporary_dir();
    let caller_path = dir.join("cmd").join("main.go");
    let service_path = dir.join("internal").join("service").join("service.go");
    fs::create_dir_all(caller_path.parent().unwrap()).unwrap();
    ...
    let method = trace_symbol_graph(&dir, "Worker::Run", TraceDirection::Callers).unwrap();
    assert_eq!(method.callers.len(), 1);
    assert_eq!(method.callers[0].symbol_id, "caller");
}
#[test]
fn keeps_go_imported_factory_methods_fail_closed_when_interface_declaration_is_ambiguous() {
    let dir = temporary_dir();
    let caller_path = dir.join("cmd").join("main.go");
    let service_dir = dir.join("internal").join("service");
    let first_path = service_dir.join("first.go");
    let second_path = service_dir.join("second.go");
    fs::create_dir_all(caller_path.parent().unwrap()).unwrap();
...
#[test]
fn keeps_go_imported_factory_methods_fail_closed_for_pointer_interface_returns() {
    let dir = temporary_dir();
    let caller_path = dir.join("cmd").join("main.go");
    let service_path = dir.join("internal").join("service").join("service.go");
    fs::create_dir_all(caller_path.parent().unwrap()).unwrap();
...
fn keeps_go_imported_concrete_factory_methods_fail_closed() {
    ...
    fs::write(
        &caller_path,
        "package main\n\nimport svc \"example.com/project/internal/service\"\n\nfunc caller() int { return svc.NewCounter().Value() }\n",
    )
    .unwrap();
    fs::write(
        &service_path,
    ...
    let method = trace_symbol_graph(&dir, "Counter::Value", TraceDirection::Callers).unwrap();
    assert!(method.callers.is_empty());
...
#[test]
fn keeps_go_imported_named_result_factory_methods_fail_closed() {
    let dir = temporary_dir();
    let caller_path = dir.join("cmd").join("main.go");
    let service_path = dir.join("internal").join("service").join("service.go");
    fs::create_dir_all(caller_path.parent().unwrap()).unwrap();
    ...
    fs::write(
        &service_path,
        "package service\n\ntype Worker interface { Run(value int) error }\nfunc NewWorker() (worker Worker) { return nil }\n",
    )
    .unwrap();
...
#[test]
fn keeps_go_imported_multi_result_factory_methods_fail_closed() {
    let dir = temporary_dir();
    let caller_path = dir.join("cmd").join("main.go");
    let service_path = dir.join("internal").join("service").join("service.go");
    fs::create_dir_all(caller_path.parent().unwrap()).unwrap();
    ...
    fs::write(
        &caller_path,
        "package main\n\nimport svc \"example.com/project/internal/service\"\n\nfunc caller() error { worker, _ := svc.NewWorker(); return worker.Run(1) }\n",
    )
    .unwrap();
    fs::write(
        &service_path,
...
#[test]
fn keeps_go_imported_ambiguous_factory_methods_fail_closed() {
    let dir = temporary_dir();
    let caller_path = dir.join("cmd").join("main.go");
    let service_dir = dir.join("internal").join("service");
    let first_path = service_dir.join("first.go");
...
#[test]
fn keeps_go_cross_file_named_result_factory_methods_fail_closed() {
    let dir = temporary_dir();
    let source_path = dir.join("caller.go");
    fs::write(
        &source_path,
...
#[test]
fn keeps_go_cross_file_factory_returns_fail_closed_when_factory_name_is_shadowed() {
    let dir = temporary_dir();
    let source_path = dir.join("caller.go");
    fs::write(
        &source_path,
        "package metrics\n\ntype Worker interface { Run(value int) error }\nfunc caller(NewWorker func() Worker) error { return NewWorker().Run(1) }\n",
    )
    .unwrap();
    fs::write(
        dir.join("factory.go"),
