...
#[test]
fn discovery_context_deadline_helpers_reject_expired_budget_before_patch_work() {
    let deadline = TraceQueryDeadline::expired_for_tests(1);
    let workspace_root = Path::new(".");
    let path = Path::new("missing.py");
    let position = Position { row: 0, column: 0 };

    let direct_error = validate_patch_with_discovery_context_with_deadline(
        workspace_root,
        path,
        "def target():\n    return 1\n",
        "target",
    ...
    .expect_err("expired trace deadline must reject direct discovery patch validation");
    assert!(direct_error.to_string().contains("patch validation"));

    let position_error = validate_patch_with_discovery_context_at_position_with_deadline(
        workspace_root,
        path,
        "def target():\n    return 1\n",
        &position,
    ...
    .expect_err("expired trace deadline must reject position discovery patch validation");
    assert!(
        position_error
            .to_string()
