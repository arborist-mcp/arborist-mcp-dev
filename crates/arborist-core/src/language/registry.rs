...
static SWIFT_DESCRIPTOR: LanguageDescriptor = LanguageDescriptor {
    id: LanguageId::Swift,
    display_name: "Swift",
...
static LUA_ADAPTER: LuaAdapter = LuaAdapter {
    syntax: SyntaxOnlyAdapter {
...
static SWIFT_ADAPTER: SwiftAdapter = SwiftAdapter {
    syntax: SyntaxOnlyAdapter {
...
impl LanguageAdapter for JavaScriptFamilyAdapter {
    ...
    fn collect_patch_reference_validation(
        ...
        crate::patching::javascript_references::collect_javascript_reference_validation_with_deadline(
            path,
            document,
            source,
            symbol_node,
    ...
    fn collect_local_file_dependencies_with_deadline(
        ...
        crate::language::javascript_local_module_dependency_paths_with_deadline(
...
fn kotlin_semantic_path_for_node(node: Node<'_>, source: &str) -> Result<Option<String>> {
    let mut root = node;
    while let Some(parent) = root.parent() {
        root = parent;
    }
    crate::semantic::kotlin::kotlin_symbol_name(node, source)?
        .map(|name| crate::semantic::kotlin::kotlin_semantic_path(root, node, source, &name))
        .transpose()
        .map(Option::flatten)
}
...
fn lua_semantic_path_for_node(node: Node<'_>, source: &str) -> Result<Option<String>> {
    let mut current = Some(node);
    while let Some(candidate) = current {
        if crate::semantic::lua::is_lua_symbol_node(candidate) {
            return crate::semantic::lua::lua_symbol_name(candidate, source)?
                .map(|name| crate::semantic::lua::lua_semantic_path(&name))
                .transpose()
                .map(Option::flatten);
        }
...
fn swift_semantic_path_for_node(node: Node<'_>, source: &str) -> Result<Option<String>> {
    let mut current = Some(node);
    while let Some(candidate) = current {
        if crate::semantic::swift::is_swift_symbol_node(candidate) {
            return crate::semantic::swift::swift_symbol_name(candidate, source)?
                .map(|name| crate::semantic::swift::swift_semantic_path(&name))
                .transpose()
                .map(Option::flatten);
        }
        current = candidate.parent();
...
impl LanguageAdapter for SwiftAdapter {
    ...
    fn build_semantic_skeleton(
        ...
        crate::semantic::swift::build_swift_skeleton(
            path,
            source,
            tree,
            depth_limit,
    ...
    fn find_semantic_node<'tree>(
        ...
        crate::semantic::swift::find_swift_semantic_node(path, tree, source, target_path, deadline)
    }
    ...
    fn position_symbol_identity(
        ...
        let semantic_path = swift_semantic_path_for_node(node, source)?.ok_or_else(|| {
            anyhow!("position does not resolve to a Swift symbol with a stable semantic path")
        })?;
        Ok(PositionSymbolIdentity {
            symbol_id: semantic_path.clone(),
            semantic_path,
    ...
    fn semantic_path_for_node(
        ...
        swift_semantic_path_for_node(node, source)
    }
...
impl LanguageAdapter for RustAdapter {
    ...
    fn collect_patch_reference_validation(
        ...
        crate::patching::rust_references::collect_rust_reference_validation_with_deadline(
            path,
            document,
            source,
            symbol_node,
    ...
    fn collect_local_file_dependencies_with_deadline(
        ...
        crate::language::rust_local_module_dependency_paths_with_deadline(
            path, root, source, deadline,
        )
        .map(|paths| paths.into_iter().collect())
    }
...
impl LanguageAdapter for GoAdapter {
    ...
    fn collect_patch_reference_validation(
        ...
        crate::patching::go_references::collect_go_reference_validation_with_deadline(
            path,
            document,
            source,
            symbol_node,
...
impl LanguageAdapter for LuaAdapter {
    ...
    fn build_semantic_skeleton(
        ...
        crate::semantic::lua::build_lua_skeleton(
            path,
            source,
            tree,
            depth_limit,
    ...
    fn find_semantic_node<'tree>(
        ...
        crate::semantic::lua::find_lua_semantic_node(path, tree, source, target_path, deadline)
    }
    ...
    fn position_symbol_identity(
        ...
        let semantic_path = lua_semantic_path_for_node(node, source)?.ok_or_else(|| {
            anyhow!("position does not resolve to a Lua symbol with a stable semantic path")
        })?;
        Ok(PositionSymbolIdentity {
            symbol_id: semantic_path.clone(),
            semantic_path,
    ...
    fn semantic_path_for_node(
        ...
        lua_semantic_path_for_node(node, source)
    }
...
impl LanguageAdapter for JavaAdapter {
    ...
    fn collect_patch_reference_validation(
        ...
        crate::patching::java_references::collect_java_reference_validation_with_deadline(
            path,
            document,
            source,
            symbol_node,
...
impl LanguageAdapter for KotlinAdapter {
    ...
    fn build_semantic_skeleton(
        ...
        crate::semantic::kotlin::build_kotlin_skeleton(
            path,
            source,
            tree,
            depth_limit,
    ...
    fn find_semantic_node<'tree>(
        ...
        crate::semantic::kotlin::find_kotlin_semantic_node(
            path,
            tree,
            source,
            target_path,
    ...
    fn ascend_to_symbol<'tree>(&self, node: Node<'tree>) -> Option<Node<'tree>> {
        let mut current = Some(node);
        while let Some(candidate) = current {
            if crate::semantic::kotlin::is_kotlin_semantic_symbol_node(candidate) {
                return Some(candidate);
            }
            current = candidate.parent();
        }
    ...
    fn position_symbol_identity(
        ...
        let semantic_path = kotlin_semantic_path_for_node(node, source)?.ok_or_else(|| {
            anyhow!("position does not resolve to a Kotlin symbol with a stable semantic path")
        })?;
        Ok(PositionSymbolIdentity {
            symbol_id: semantic_path.clone(),
            semantic_path,
    ...
    fn semantic_path_for_node(
        ...
        kotlin_semantic_path_for_node(node, source)
    }
    ...
    fn patch_replacement_node<'tree>(&self, node: Node<'tree>) -> Node<'tree> {
        crate::semantic::kotlin::kotlin_patch_replacement_node(node)
    }
    ...
    fn replacement_preserves_required_wrappers(
        ...
        // Kotlin modifiers and annotations are inside the replaced declaration node, so the
        // replacement either preserves or intentionally replaces them; no outer wrapper is
        // silently dropped.
        true
    }
    ...
    fn collect_patch_reference_validation(
        ...
        crate::patching::kotlin_references::collect_kotlin_reference_validation_with_deadline(
            path,
            document,
            source,
            symbol_node,
    ...
    fn collect_local_file_dependencies(
        ...
        crate::language::kotlin_local_file_dependency_paths(path, root, source)
            .map(|paths| paths.into_iter().collect())
    }
    ...
    fn collect_local_file_dependencies_with_deadline(
        ...
        crate::language::kotlin_local_file_dependency_paths_with_deadline(
            path, root, source, deadline,
        )
        .map(|paths| paths.into_iter().collect())
    }
...
impl LanguageAdapter for CSharpAdapter {
    ...
    fn patch_replacement_node<'tree>(&self, node: Node<'tree>) -> Node<'tree> {
        crate::semantic::csharp::csharp_patch_replacement_node(node)
    }
    ...
    fn replacement_preserves_required_wrappers(
        ...
        // C# attributes and modifiers are inside the replaced declaration node, so the
        // replacement either preserves or intentionally replaces them; no outer wrapper is
        // silently dropped.
        true
    }
...
impl LanguageAdapter for PythonAdapter {
    ...
    fn collect_local_file_dependencies_with_deadline(
        ...
        crate::language::python_local_file_dependency_paths_with_deadline(
            path, root, source, deadline,
        )
        .map(|paths| paths.into_iter().collect())
    }
