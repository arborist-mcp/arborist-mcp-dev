...
fn csharp_foreach_collection_element_type(
    ...
    if node.kind() == "parenthesized_expression" {
        let Some(inner) = csharp_parenthesized_inner_expression(node) else {
            return Ok(None);
        };
    ...
    let spelling = match node.kind() {
        ...
        "conditional_access_expression" => {
            ...
            // type.
            let text = crate::language::node_text(node, source)?
                .trim()
                .replace("?.", ".");
            if text.is_empty() {
                return Ok(None);
...
fn csharp_foreach_statement_is_await(node: Node<'_>) -> bool {
    let mut cursor = node.walk();
