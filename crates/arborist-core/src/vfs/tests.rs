...
fn traces_cpp_standard_smart_pointer_member_calls_from_unsaved_virtual_changes() {
    ...
    vfs.open_file(
        &source,
        Some(
            "namespace api { class Counter { public: int adjust(int value) & { return value; } int adjust(int value) const & { return value + 1; } int adjust(int value) && { return value + 2; } }; struct Deleter {}; using Alias = Counter; int unique_caller(int value) { std::unique_ptr<Alias> current; return current->adjust(value); } int unique_get_caller(int value) { std::unique_ptr<Alias> current; return current.get()->adjust(value); } int auto_unique_caller(int value) { auto current = std::unique_ptr<Alias>{}; return current->adjust(value); } int moved_unique_dereference_caller(int value) { std::unique_ptr<Alias> current; return (*std::move(current)).adjust(value); } int reference_wrapper_get_caller(int value) { Alias target{}; std::reference_wrapper<Alias> current(target); return current.get().adjust(value); } int const_reference_wrapper_get_caller(int value) { const Alias target{}; std::reference_wrapper<const Alias> current(target); return current.get().adjust(value); } int optional_arrow_caller(int value) { std::optional<Alias> current; return current->adjust(value); } int auto_optional_arrow_caller(int value) { auto current = std::optional<Alias>{}; return current->adjust(value); } int nested_optional_unique_arrow_caller(int value) { std::optional<std::unique_ptr<Alias>> current; return (*current)->adjust(value); } int moved_optional_arrow_caller(int value) { std::optional<Alias> current; return std::move(current)->adjust(value); } int const_optional_arrow_caller(int value) { std::optional<Alias> current; return std::as_const(current)->adjust(value); } int optional_dereference_caller(int value) { std::optional<Alias> current; return (*current).adjust(value); } int moved_optional_value_caller(int value) { std::optional<Alias> current; return std::move(current).value().adjust(value); } int moved_optional_dereference_caller(int value) { std::optional<Alias> current; return (*std::move(current)).adjust(value); } int const_optional_value_caller(int value) { const std::optional<Alias> current{}; return current.value().adjust(value); } int const_optional_dereference_caller(int value) { const std::optional<Alias> current{}; return (*current).adjust(value); } int addressof_caller(int value) { Alias current{}; return std::addressof(current)->adjust(value); } int const_addressof_caller(int value) { const Alias current{}; return std::addressof(current)->adjust(value); } int custom_unique_caller(int value) { std::unique_ptr<Alias, Deleter> current; return current->adjust(value); } int shared_caller(int value) { std::shared_ptr<Alias> current; return current->adjust(value); } int const_unique_caller(int value) { std::unique_ptr<const Alias> current; return current->adjust(value); } int const_unique_get_caller(int value) { std::unique_ptr<const Alias> current; return current.get()->adjust(value); } }\n",
        ),
    )
    .unwrap();
